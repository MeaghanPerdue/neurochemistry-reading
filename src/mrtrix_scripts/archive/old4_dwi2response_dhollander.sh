...
#do NOT use the upsampled images for this, just the bias corrected, upsampled images will be used later
...
for subj in $(cat subjlist.txt)
	do
	cd $mrtrix_out/$subj
	dwi2response dhollander -voxels voxels.mif dwi_preproc_biascorr.mif wm_response.txt gm_response.txt csf_response.txt
	done
